...
mod circuits {
    ...
    /// - swap_amount: Amount being swapped (plaintext, already validated by ZK proof)
    ...
    pub fn confidential_swap(
        ...
        vault_state: Enc<Mxe, VaultState>,
        user_position: Enc<Mxe, UserPosition>,
        swap_amount: u64,           // Plaintext (validated by ZK proof)
        ...
        if should_execute {
            ...
            if position.deposited >= swap_amount {
                position.deposited -= swap_amount;
            }
            // Update vault liquidity
            if vault.total_deposited >= swap_amount {
                vault.total_deposited -= swap_amount;
            }
        }
    ...
    pub fn process_dca(
        ...
        current_price: u64,
    ) -> (Enc<Shared, SwapResult>, Enc<Shared, DCAConfig>) {
        ...
        // Update remaining swaps if executing
        if should_execute {
            config.swaps_remaining -= 1;
        }
